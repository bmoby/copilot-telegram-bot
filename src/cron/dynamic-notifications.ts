...
export async function ensureDailyPlan(): Promise<void> {
  ...
  try {
    ...
    logger.info(
      { taskCount: livePlan.length },
      "Daily live plan auto-generated",
    );
  } catch (error) {
...
export async function planDay(): Promise<number> {
  try {
    ...
    if (cancelled > 0) {
      logger.info(
        { cancelled },
        "Cancelled stale notifications before replanning",
      );
    }
    ...
    const reminders = planned
      .map((notif) => {
        ...
        return {
          ...
          repeat_config: {
            type: notif.type,
            planned_by: "notification-planner",
          },
    ...
    logger.info(
      { stored: reminders.length, total: planned.length },
      "Daily notifications stored in DB",
    );
...
export async function getNotificationsSummary(): Promise<string> {
  ...
  if (active.length > 0) {
    ...
    for (const r of active.slice(0, 5)) {
      const time = new Date(r.trigger_at).toLocaleTimeString("ru-RU", {
        hour: "2-digit",
        minute: "2-digit",
      });
