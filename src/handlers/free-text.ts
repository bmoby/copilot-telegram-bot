  ...
  bot.on("message:text", async (ctx: Context) => {
    ...
    if (isOnboarding(chatId)) {
      try {
        ...
        await ctx.reply(
          "Ошибка во время знакомства. Отправь сообщение ещё раз.",
        );
      }
      return;
    }
    ...
    try {
      ...
      const reorgAction = result.actions.find(
        (a) => a.type === "reorganize_plan",
      );
      ...
      if (reorgAction) {
        ...
        try {
          ...
          const pending = reorgResult.livePlan.filter(
            (t) => t.status === "pending" || t.status === "in_progress",
          );
          const done = reorgResult.livePlan.filter((t) => t.status === "done");
          const deferred = reorgResult.livePlan.filter(
            (t) => t.status === "deferred",
          );
          ...
          if (deferred.length > 0) {
            ...
            for (const t of deferred)
              reorgMessage += `  ${t.title} → ${t.deferred_to}\n`;
          }
          ...
          await ctx.reply(
            "Ошибка при реорганизации. План остаётся без изменений.",
          );
        }
        return;
      }
      ...
      await ctx.reply("Ошибка обработки. Отправь сообщение ещё раз.");
    }
  });
}
